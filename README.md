...
## Discussion
The results of our study demonstrate the effectiveness of machine learning models in predicting diabetes and identifying key risk factors. By employing multiple algorithms—Logistic Regression, K-Nearest Neighbors (KNN), Decision Tree, Random Forest, and Neural Network—we gained a comprehensive understanding of the strengths and limitations of each approach. Among these, Random Forest emerged as the best-performing model, achieving an ROC AUC of 0.8254 and a Cross-Validation Mean of 0.7737, highlighting its robustness and reliability. The superior performance of Random Forest can be attributed to its ability to capture complex, non-linear relationships and reduce overfitting through ensemble learning.

Feature importance analysis revealed that Glucose, BMI, and Age are the most critical predictors of diabetes. These findings align with existing medical literature, reinforcing the physiological and demographic risk factors associated with diabetes. Glucose, in particular, emerged as the most significant feature, reflecting its well-established role in diabetes diagnosis and management. BMI and Age also contributed significantly, underscoring the importance of weight management and age-related monitoring in diabetes prevention strategies.

While the Random Forest model performed well overall, other models like Logistic Regression and KNN provided additional insights. For example, Logistic Regression achieved the highest accuracy (75.32%), demonstrating its simplicity and effectiveness as a baseline model. However, its tendency to underperform on recall highlights the challenge of imbalanced datasets, where the positive class (diabetic cases) is underrepresented. Similarly, KNN's performance emphasized the importance of hyperparameter tuning, with the Manhattan distance and optimized neighbor count (k=8) enhancing its predictive capability.

Despite our promising results, certain limitations must be acknowledged. The dataset we used, while useful, is relatively small and imbalanced. This may influence model performance and lead to overfitting or results that lack generalizability. Additionally, some features, such as Insulin and SkinThickness, exhibited missing or skewed values, potentially limiting their predictive power.
...
Among the models tested, Random Forest was selected as the optimal model due to its superior performance, achieving ROC AUC of 0.8254 and robust cross-validation results. This model's ability to balance recall and precision makes it particularly valuable for identifying individuals at risk of diabetes.
